...
//     const startTime = selectedSlot ? selectedSlot.time : convertTo12Hour(bookingData.startTime);
//     const endTime = convertTo12Hour(bookingData.endTime);
...
//         const timeSlot = `${hour12}${minutes === 0 ? '' : ':30'} ${period}`;
//         slots.push(timeSlot);
...
//     const timeSlots = generateTimeSlots(bookingData.startTime, bookingData.endTime);
...
//   return (
//     <div className="calendar-container-with-specifications">
...
//       {/* <div>Debug: userRole={userRole}, isAdmin={isAdmin.toString()}</div> */}
...
{/* {console.log("Rendering heading and button, isAdmin:", isAdmin)} */}
  ...
  {timeSlots.map((time) => (
    <tr key={time} className="time-row">
      ...
      {filteredRooms.map(room => {
        ...
        const halfHourKey = `${room.id}-${time.split(':')[0]}:30 ${time.split(' ')[1]}`;
        ...
        return (
          <td key={`${room.id}-${time}`} className="time-slot-container">
            <div
              className={`time-slot-half ${
                isPending ? 'pending' : 
                isBooked || bookings[halfHourKey]?.status === 'active' ? 'booked' : ''
              }`}
              onClick={() => handleSlotClick(room, time)}
            >
              {bookings[slotKey]?.title || pendingBookings[selectedDate]?.[slotKey]?.title || ''}
            </div>
            <div
              className={`time-slot-half ${
                isPending ? 'pending' : 
              ...
              onClick={() => handleSlotClick(room, `${time.split(':')[0]}:30 ${time.split(' ')[1]}`)}
            >
              {bookings[halfHourKey]?.title || pendingBookings[selectedDate]?.[halfHourKey]?.title || ''}
            </div>
          </td>
        );
      })}
